...
class ApiAuthFilter implements FilterInterface
{
    public function before(RequestInterface $request, $arguments = null)
    {
        ...
        $row = $db->table('api_tokens t')
            ->select('t.*, u.id AS user_id, u.fullname, u.email, u.role_id, r.name AS role_name')
        ...
        // Store in session for controllers to access
        session()->set('api_user', $row);
    }

    public function after(RequestInterface $request, ResponseInterface $response, $arguments = null)
    {
        // Clear api_user from session after request
